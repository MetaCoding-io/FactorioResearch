...
| RV-006 | 0007 | Adjacent checkpoint observations can determine actual crafting progress across normal progress, craft completion/reset, and high effective craft speed. | Record `crafting_progress`, `products_finished`, raw status, recipe and tick over controlled machines. | Interval classifier recognizes positive progress through completion/reset without false idle/productive intervals. | API evidence confirmed on Factorio 2.0.77 (`test_rv006_craft_progress_probe`: `products_finished` monotonic, `crafting_progress` ∈ [0,1], completions observable). Interval classifier implemented (`fisl/classify.lua` + `fisl/machine_state.lua`, issue #7) with `products_finished`-first completion-wrap handling; per-state fixtures in `tests/integration/test_machine_state.py` — execution against the real runtime pending. |
| RV-007 | 0007 | Brownout/low-power operation can be distinguished as progressing + energy-limited versus stopped/unavailable at the required resolution. | Controlled power-limited crafting fixture. | Progress evidence and raw status support the ADR 0007 two-dimensional classification. | Fixture written (`test_rv007_brownout_stays_productive_with_energy_limited`: undersized EEI, expects productive + energy_limited on `low_power`); execution against the real runtime pending. |

...
| RV-012 | 0016 | Dynamic production-entity membership can be maintained incrementally from runtime events with canonical eligibility boundaries. | Build/remove matching machines during a run and record membership intervals. | New/removed machines enter/leave pooled denominators on the intended FISL boundaries. | **Confirmed on Factorio 2.0.77** (2026-08-26): `test_rv012_membership_add_and_remove_mid_run` — a machine built mid-run (script_raised_built) joined at the drain boundary with no retroactive history, its later destruction ended eligibility at the removal boundary, spans exactly partitioned the eligibility interval, and pooled state-fraction denominators summed eligibility ∩ window. Full integration suite (16/16) re-validated the RV-006/RV-007 fixtures against the dynamic runtime in the same sweep. |

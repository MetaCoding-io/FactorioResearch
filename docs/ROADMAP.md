...
| 6 | Lab content: builder generalization, Labs 0–2/4/6, RV partial fixtures, per-lab reference solutions + course chapters — **in progress: builder generalized (layout registry); Lab 4 complete as a teachable unit** (baseline + verification + comparison data with state fractions committed; chapter cites measured runs incl. the pooled-productive-vs-throughput inversion; remaining: lab-04 screenshots via `fisl snap`, Labs 0–2/5–6, RV-004 underground/splitter fixtures, external-learner gate) | [#11](https://github.com/MetaCoding-io/FactorioResearch/issues/11) | the rest of the course |

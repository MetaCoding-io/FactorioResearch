...
| 6 | Lab content: builder generalization, Labs 0–2/4/6, RV partial fixtures, per-lab reference solutions + course chapters — **in progress: builder generalized (layout registry), Lab 4 authored** (fp04 scenario + two reference solutions + chapter + snap set; remaining: `fisl build-baseline scenarios/factory-physics/fp04-starvation-blocking`, then `fisl solutions … --run` for the reference dataset, commit baseline + verification + comparison data) | [#11](https://github.com/MetaCoding-io/FactorioResearch/issues/11) | the rest of the course |

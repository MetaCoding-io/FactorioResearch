...
| 4 | Demand/service cohorts + on-time item rate (ADR 0008) — **implemented, incl. fp05 + chapter; remaining: run `FACTORIO_BIN=… python3 -m pytest tests/integration/test_demand_service.py`, `fisl build-baseline scenarios/factory-physics/fp05-push-and-pull`, `fisl solutions … --run`, commit evidence/baseline/data** | [#9](https://github.com/MetaCoding-io/FactorioResearch/issues/9) | Lab 5 push/pull |
...
| 6 | Lab content: builder generalization, Labs 0–2/4/6, RV partial fixtures, per-lab reference solutions + course chapters — **in progress: builder generalized (layout registry); Lab 4 complete as a teachable unit; Labs 0–2 authored** (fp00/fp01/fp02 scenarios + chapters + fp02 upgrade-pair solutions + entry-boundary `admission_rate` metric; remaining: `fisl build-baseline` each of fp00/fp01/fp02, `fisl solutions scenarios/factory-physics/fp02-the-constraint --run --json course/data/lab-02-comparison.json --svg …`, cite measured numbers, screenshots, Labs 5–6, RV-004 underground/splitter fixtures, external-learner gate) | [#11](https://github.com/MetaCoding-io/FactorioResearch/issues/11) | the rest of the course |

...
| 6 | Lab content: builder generalization, Labs 0–2/4/6, RV partial fixtures, per-lab reference solutions + course chapters — **in progress: builder generalized (layout registry); Lab 4 built & verified on real 2.0.77** (baseline TH 30/min exact; buffer solution +58% WIP at +0% TH; upgrade solution +25% TH — chapter cites measured runs; remaining: commit fp04 baseline + verification summary from the executing machine, rerun `fisl solutions … --run --json --svg` once so the comparison data includes the state-fraction rows, lab-04 screenshots, then Labs 0–2/5–6) | [#11](https://github.com/MetaCoding-io/FactorioResearch/issues/11) | the rest of the course |

...
::: {.callout-tip}
## Hands you'll need
First lab that builds: placing entities from the toolbox, and — for the
pull gate — a circuit wire with an enable condition. Practice Range
drills `d5_enable_condition` (and `d1`/`d2` if belts are new to you).
:::
...
![The FISL panel in READY: the experiment waits for you to press Start.](../images/lab-03/fisl-panel-ready.jpg){#fig-ready width=60%}
...
![Run 1, mid-experiment: the input belt packed solid with waiting workpieces — this queue *is* the WIP number on the panel.](../images/lab-03/run1-jammed-belt.jpg){#fig-jam}
...
![The pull gate: red wire from the source inserter to the last belt tile before machine 1.](../images/lab-03/run2-pull-gate.jpg){#fig-gate}
...
avg WIP 41.66 instead of 19.16. Diagnosing a control system by *walking the
...
- Baseline reference numbers come from the committed verification run
  `01M10GTKR13N2RCW3AYCPVFS65` (2026-08-27 baseline revision). Measured
  pull-solution result (run `01M10GZ303RBTWMF2PWA02CD3P`, solution v3): TH
  unchanged at 15.00/min, avg WIP 19.16 (−62.9%), CT 76.64 s — right on
  the transit floor predicted in the solution README. Every number in this
  chapter has now reproduced identically across two independently built
  baselines and multiple machines (the original verified run was
  `01M0XP8VJ78KXRC8DB6BATBW39`; a human-played run matched it
  bit-for-bit).

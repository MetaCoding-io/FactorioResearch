...
What actually happens (run it before believing prose):
...
- **A** makes M1's blocked fraction collapse and its productive fraction
  ...
  M2 never stopped being the constraint, and a buffer feeds no one faster.
  The surplus M1 produces now accumulates in the chest for the entire run,
  so WIP climbs and climbs, and cycle time with it (Little's Law is not
  optional). You improved a machine and worsened every workpiece.
- **B** moves throughput — up ~25%, to the *next* constraint (M1's
  1.6 s ⇒ 37.5/min). And watch the state signature migrate: M1's blocked
  time vanishes, while M2 and M3 now show starved time. Walk the line
  again: blocked-above/starved-below now points at M1. The method still
  works; the answer moved, because the constraint moved.
...
- Reference interventions are committed as
  ...
  --json course/data/lab-04-comparison.json`. Cite measured numbers in
  this chapter after the first verified run; the rate arithmetic above
  (30/min baseline, 37.5/min after B) is the prediction to check.
